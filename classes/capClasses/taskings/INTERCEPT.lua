...
function INTERCEPT.getMissionFromAirbase(groupName, interceptPoint, airbase, config)
    ...
    local pointA, pointB, pointC, pointD = INTERCEPT.getInterceptTaskPoint(groupName, point, interceptPoint, airbaseVec2, config)
...
---@param config CapConfig
---@return table
function INTERCEPT.getMissionFromInAir(groupName, currentPosition, interceptPoint, airbase, config)

    local pointAirbasev3 = airbase:getPoint()
    local airbase2 = { x = pointAirbasev3.x, y = pointAirbasev3.z }

    local pointA, pointB = INTERCEPT.getInterceptTaskPoint(groupName, currentPosition, interceptPoint, airbase2, config)
...
function INTERCEPT.getInterceptTaskPoint(groupName, currentPoint, targetPoint, airbasePoint, config)

    local pointA = {
        alt = config:getMaxAlt(),
